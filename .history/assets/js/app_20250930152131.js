...
function loadGrowthData() {
  ...
  // Update growth chart (simplified - in real app use Chart.js)
  const chartContainer = document.getElementById("growthChart");
  if (chartContainer && animalGrowthData.length > 0) {
    // Placeholder for chart - implement with Chart.js
    const ctx = chartContainer.getContext('2d');
    ctx.clearRect(0, 0, chartContainer.width, chartContainer.height);
    ctx.fillText("Grafik Pertumbuhan akan ditampilkan di sini", 50, 100);
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  upcomingList.innerHTML = upcoming.map(schedule => `
    <div class="schedule-item">
      ...
      <div class="schedule-details">
        <h5>${schedule.type}</h5>
        <p>${schedule.animalName} - ${schedule.description}</p>
      </div>
    </div>
  `).join('');
}
