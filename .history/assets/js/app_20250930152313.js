...
function loadRecentActivities() {
  ...
  if (activities.length === 0) {
    container.innerHTML = '<div class="no-data">Belum ada aktivitas terbaru</div>';
    return;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (animalForm) {
    animalForm.addEventListener("submit", function (e) {
      ...
      animals.push(animal);
      addActivity('animal_added', `Hewan baru ${animal.name} (${animal.species}) telah ditambahkan`, animal.id);
      saveData();
  ...
  if (vaccinationForm) {
    vaccinationForm.addEventListener("submit", function(e) {
      ...
      addActivity('medical_record', `Vaksinasi ${vaccination.vaccineName} untuk ${animal?.name} telah dicatat`, parseInt(vaccination.animalId));
  ...
  if (scheduleForm) {
    scheduleForm.addEventListener("submit", function(e) {
      ...
      addActivity('care_scheduled', `Jadwal ${schedule.type} untuk ${animal?.name} telah dibuat`, parseInt(schedule.animalId));
  ...
  if (growthForm) {
    growthForm.addEventListener("submit", function(e) {
      ...
      const selectedAnimalId = document.getElementById("selectAnimalMonitoring").value;
      if (!selectedAnimalId) {
      ...
      addActivity('growth_recorded', `Data pertumbuhan ${animal?.name} telah diperbarui`, parseInt(selectedAnimalId));
  ...
  if (harvestForm) {
    harvestForm.addEventListener("submit", function(e) {
      ...
      addActivity('harvest_recorded', `Hasil panen ${harvest.type} sebanyak ${harvest.quantity} ${harvest.unit} telah dicatat`);
...
function updateDashboard() {
  ...
  if (healthyAnimalsEl) {
    const healthyCount = animals.filter(animal => animal.status === "Aktif").length;
  ...
  if (monthlyRevenueEl) {
    ...
    monthlyRevenueEl.textContent = `Rp ${monthlyRevenue.toLocaleString('id-ID')}`;
  }
}
...
function calculateAverageGrowthRate() {
  ...
  animals.forEach(animal => {
    ...
    if (animalGrowth.length >= 2) {
      ...
      if (daysDiff > 0) {
        const dailyGrowthRate = (weightGain / firstRecord.weight) * 100 / daysDiff;
...
function clearMonitoringData() {
  ...
  if (infoContainer) infoContainer.innerHTML = '<div class="no-data">Pilih hewan untuk melihat detail</div>';
  if (chartContainer) chartContainer.innerHTML = '<div class="no-data">Pilih hewan untuk melihat grafik pertumbuhan</div>';
  if (analysisContainer) analysisContainer.innerHTML = '<div class="no-data">Pilih hewan untuk melihat analisis</div>';
...
function updateGrowthChart(data) {
  ...
  if (data.length === 0) {
    chartContainer.innerHTML = '<div class="no-data">Belum ada data pertumbuhan</div>';
    return;
  }
...
function updateGrowthAnalysis(growthData) {
  ...
  if (!analysisContainer || growthData.length < 2) {
    if (analysisContainer) {
      analysisContainer.innerHTML = '<div class="no-data">Data pertumbuhan tidak cukup untuk analisis</div>';
    }
    return;
  }
...
function loadCareSchedules() {
  ...
  if (upcoming.length === 0) {
    upcomingList.innerHTML = '<div class="no-data">Tidak ada jadwal perawatan mendatang</div>';
    return;
  }
  ...
  upcomingList.innerHTML = upcoming.map(schedule => {
    ...
    return `
      <div class="schedule-item">
        <div class="schedule-date">${new Date(schedule.date).toLocaleDateString('id-ID')}</div>
        <div class="schedule-details">
          <div class="schedule-type">${schedule.type}</div>
          <div class="schedule-animal">${animal ? animal.name : 'Hewan tidak ditemukan'}</div>
...
function updateKPIDashboard() {
  ...
  if (milkProductionEl) {
    const todayMilk = harvestData
      .filter(h => h.type === "Susu" && h.date === new Date().toISOString().split('T')[0])
  ...
  if (eggProductionEl) {
    const todayEggs = harvestData
      .filter(h => h.type === "Telur" && h.date === new Date().toISOString().split('T')[0])
      .reduce((total, h) => total + h.quantity, 0);
