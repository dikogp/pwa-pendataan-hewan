...
function exportAllData() {
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(dataBlob);
  link.download = `hewanku-data-backup-${new Date().toISOString().split('T')[0]}.json`;
  link.click();
}
...
function generateMonthlyReport() {
  ...
  const monthlyData = {
    ...
    health: {
      ...
      vaccinationsGiven: (careSchedules || []).filter(s => 
        s.type === 'Vaksinasi' && 
      ...
      <body>
        ...
        <div class="section">
          ...
          <table class="stats-table">
            ...
            <tr><td>Penambahan Bulan Ini</td><td>${monthlyData.animals.newAdditions}</td></tr>
          </table>
        </div>
        ...
        <div class="section">
          ...
          <table class="stats-table">
            <tr><th>Metrik</th><th>Nilai</th></tr>
            <tr><td>Total Pemeriksaan</td><td>${monthlyData.health.totalCheckups}</td></tr>
            <tr><td>Vaksinasi Diberikan</td><td>${monthlyData.health.vaccinationsGiven}</td></tr>
          </table>
        </div>
...
function generateAnnualReport() {
  ...
  const annualData = {
    ...
    animals: {
      total: animals.length,
      newAdditions: animals.filter(a => new Date(a.dateAdded).getFullYear() === currentYear).length,
      bySpecies: animals.reduce((acc, animal) => {
        acc[animal.species] = (acc[animal.species] || 0) + 1;
        return acc;
      }, {})
    },
    totalProduction: (harvestData || []).filter(h => new Date(h.date).getFullYear() === currentYear),
    revenue: calculateYearlyRevenue(currentYear),
    growth: calculateYearlyGrowthStats()
  };
  
  alert(`Laporan tahunan ${currentYear} akan didownload. Total hewan: ${annualData.animals.total}, Pendapatan: Rp ${annualData.revenue.toLocaleString('id-ID')}`);
}
...
function generateHealthReport() {
  const healthStats = {
    ...
    upcomingCare: (careSchedules || []).filter(s => new Date(s.date) > new Date()).slice(0, 10)
  };
  
  alert(`Laporan kesehatan: ${healthStats.healthyAnimals}/${healthStats.totalAnimals} hewan sehat, ${healthStats.sickAnimals} sakit, ${healthStats.quarantineAnimals} karantina`);
}
...
function generateFinancialReport() {
  ...
  alert(`Laporan keuangan: Pendapatan tahunan Rp ${revenue.toLocaleString('id-ID')}, Bulanan Rp ${monthlyRevenue.toLocaleString('id-ID')}`);
}
...
function generateCSVContent() {
  const headers = ['ID', 'Nama', 'Spesies', 'Ras', 'Jenis Kelamin', 'Umur', 'Berat', 'Status', 'Lokasi', 'Tanggal Ditambahkan'];
...
function exportToBI() {
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(dataBlob);
  link.download = `hewanku-bi-export-${new Date().toISOString().split('T')[0]}.json`;
  link.click();
}
